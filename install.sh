...
apt-get install xfce4 gnome-terminal nautilus dbus termux-x11 -y
echo "#!/data/data/com.termux/files/usr/bin/bash export DISPLAY=:0 startxfce4" >> $PREFIX/bin/x11start
echo "#!/data/data/com.termux/files/usr/bin/bash pkill -f startxfce4 pkill -f X11" >> $PREFIX/bin/x11stop
...
echo "wget https://raw.githubusercontent.com/L234m/Mod-Distro/refs/heads/main/theme.sh ; bash  theme.sh; exit" >> $CHROOT/home/ubuntu/.bashrc
ubuntu
...
echo
echo "x11start  -  To start x11server, Execute inside ubuntu"
echo
echo "x11stop  -  To stop x11server, Execute inside ubuntu"${W}
rm -rf ~/install.sh
}
